...
static_ips: 
    - <font color="red">10.10.7.62</font>                  # 사용할 IP addresses 정의(필수): MySQL 서버 IP
    #- 10.30.40.166
    #- 10.30.40.167
...
cluster_ips:                   # 클러스터 구성시 IPs(필수)
    - <font color="red">10.10.7.62</font>                  # MySQL 서버 IP
    #- 10.30.40.166
    #- 10.30.40.167
...
static_ips: <font color="red">10.10.7.63</font>                    # 사용할 IP addresses 정의(필수): Proxy IP
  properties:
cluster_ips:
    - <font color="red">10.10.7.62</font>                         # MySQL 서버 IP
    #- 10.30.40.166
...
nats:                             # CF 설치시 설치한 nats 정보 (필수)
      machines:
      - <font color="red">10.10.3.11</font># nats 서버 IP
...
- instances: 1
  ...
  - name: openpaas_network
static_ips: 
    - <font color="red">10.10.7.61</font># 사용할 IP addresses 정의(필수): 서비스 브로커 IP
    #- 10.30.40.165
  properties:
auth_password: <font color="red">admin</font>             # 브로커 접근 아이디 비밀번호(필수)
auth_username: <font color="red">admin</font># 브로커 접근 아이디(필수)
cc_api_uri: <font color="red">http://api.controller.open-paas.com</font># CF 설치시 설정한 apiuri 정보(필수)
cookie_secret: <font color="red">admin</font>
    #external_host: <font color="red">10.10.7.61</font># 사용할 IP addresses 정의(필수): 서비스 브로커 IP
...
admin_password: admin
      host: <font color="red">10.10.7.63</font># proxy IP(필수)
persistent_disk: 10000           # 영구적 디스크 사이즈 정의(옵션): 10G
nats:                        # CF 설치시 설정하는 Nats 정보 (필수): 위와 동일
      machines:
      - <font color="red">10.10.3.11</font>
...
broker_network: openpaas_network
    ...
    - dashboard_client:                 # 서비스 dashboard client 정보
      ...
      metadata:
displayName: MySQL for Pivotal Cloud Foundry         # 서비스 아이디(고유식별자)
documentationUrl: http://docs.controller.open-paas.com/p-mysql/  # 문서제공되는Url
...
supportUrl: https://support.controller.open-paas.com   # 서포트 가능한 Url
      name: p-mysql                                 # 서비스 이름
...
- instances: 1
  ...
  properties:
    broker:# 서비스 브로커 설정 정보
      host: <font color="red">10.10.7.61</font># 서비스 브로커 IP
      name: p-mysql
      password: admin
      username: admin
      protocol: http
...
- name: openpaas_network
  subnets:
  - cloud_properties:
net_id: <font color="red">bfef7fe1-bf86-4e29-842e-84a4d1cff110</font># OpenStack에서 사용하는 network 이름 아이디(필수)
security_groups:
      - <font color="red">cf-security</font># OpenStack 에서 사용하는 접근 시큐리티 이름 이름(필수)
dns:                      # DNS 정보
    - <font color="red">10.10.5.108</font>
    - <font color="red">8.8.8.8</font>
    gateway: <font color="red">10.10.7.1</font>
range: <font color="red">10.10.7.0/24</font>
    reserved:                  # 설치시 제외할 IP 설정 (OpenStack 에서는 서비스팩 설치 구간 이외에는 IP 제외 설정을 해줘야 오류가 나지 않음. 
    - <font color="red">10.10.7.2 - 10.10.7.60</font>
    #- 10.30.40.1 - 10.30.40.100
    #- 10.30.40.201 - 10.30.254.254
    static:
    - <font color="red">10.10.7.61 - 10.10.7.70</font>#사용 가능한 IP 설정
  type: manual
properties: {}
...
stemcell:
    name: <font color="red">bosh-openstack-kvm-ubuntu-trusty-go_agent</font>#stemcell 이름(필수)
...
![mysql_openstack_4.1.07]<br><br>

-	설치를 위한 모든 설정이 한번에 출력된다.확인 후 Install 버튼을 클릭하여 설치를 진행한다.
![mysql_openstack_4.1.08]<br><br>

-	Finish 버튼 클릭으로 설치를 완료한다.  
![mysql_openstack_4.1.09]<br><br>

-	HeidiSQL을 실행했을 때 처음 뜨는 화면이다. 이 화면에서 Server에 접속하기 위한 profile을 설정/저장하여 접속할 수 있다. 신규 버튼을 클릭한다.  
![mysql_openstack_4.1.10]<br><br>

-	어떤 Server에 접속하기 위한 Connection 정보인지 별칭을 입력한다.  
![mysql_openstack_4.1.11]<br><br>

-	네트워크 유형의 목록에서 MySQL(SSH tunel)을 선택한다.
![mysql_openstack_4.1.12]<br><br>

-	아래 붉은색 영역에 접속하려는 서버 정보를 모두 입력한다.  
![mysql_openstack_4.1.13]<br>
서버 정보는 Application에 바인드되어 있는 서버 정보를 입력한다. cf env <app_name> 명령어로 이용하여 확인한다.  
**예)** $cf env hello-tomcat-mysql
![mysql_openstack_4.1.14]<br><br>

-	SSH 터널 탭을 클릭하고 OpenPaaS 운영 관리자에게 제공받은 SSH 터널링 가능한 서버 정보를 입력하고 개인 키 파일을 불러온다. plink.exe 위치 입력은 Putty에서 제공하는 plink.exe 실행 위치를 넣어주고 만일 해당 파일이 없을 경우 plink.exe 내려받기 링크를 클릭하여 다운받는다. 로컬 포트 정보는 임의로 넣고 열기 버튼을 클릭하면 Mysql 데이터베이스에 접속한다.
(참고) 만일 개인 키 없이 ID/Password로 접속이 가능한 경우에는 vsphere용 Open PaaS Mysql 서비스팩 설치 가이드를 참고한다.
![mysql_openstack_4.1.15]<br><br>
...
![mysql_openstack_4.1.16]<br><br>

-	우측 화면에 쿼리 탭을 클릭하여 Query문을 작성한 후 실행 버튼(삼각형)을 클릭한다.
쿼리문에 이상이 없다면 정상적으로 결과를 얻을 수 있을 것이다.
![mysql_openstack_4.1.17]<br><br>
