...
2. 서비스 설정을 위한 properties 파일을 작성한다.
    ...
    알 수 없는 스킴이면 UriBasedServiceInfo 클래스를 기본으로
    하여 활성화된다.
...
3. maven-failsafe-plugin 설정
    ...
    환경변수를 설정할 수 있다. 실제 테스트 수행시 서비스 환경에 맞게
    VCAP_SERVICES의 값을 변경해야 한다.
    -   Spring.cloud.propertiesFile 시스템 프로퍼티를 등록하여 설정파일의
    위치를 지정해야 한다.
